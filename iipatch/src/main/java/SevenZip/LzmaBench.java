...
public class LzmaBench {
	...
	static public int LzmaBenchmark(int numIterations, int dictionarySize) throws Exception {
		...
		if (!encoder.SetDictionarySize(dictionarySize)) throw new Exception("Incorrect dictionary size");
		...
		for (int i = 0; i < numIterations; i++) {
			...
			for (int j = 0; j < 2; j++) {
				...
				if (!decoder.Code(inputCompressedStream, crcOutStream, outSize)) throw (new Exception("Decoding Error"));
				;
